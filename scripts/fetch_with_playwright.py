...
async def main():
    ...
    parser.add_argument("--output", type=Path, default=Path("training_data/playwright_fetch"))
    ...
    async with async_playwright() as p:
        ...
        try:
            response = await page.goto(auction_url, wait_until="networkidle", timeout=30000)
        ...
        for idx, lot in enumerate(lots[:args.limit]):
            ...
            try:
                response = await page.goto(lot.url, wait_until="networkidle", timeout=30000)
        ...
        print(f"\nNext steps:")
        print(f"  1. Process HTML files:")
        print(f"     python scripts/process_saved_lot_pages.py --html-dir {args.output}")
        print(f"  2. Or download images directly:")
        print(f"     python scripts/download_images.py --urls {args.output}/image_urls.json")
