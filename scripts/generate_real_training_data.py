...
async def fetch_lot_detail(client: httpx.AsyncClient, url: str) -> dict | None:
    ...
    Args:
        ...
        url (str): Lot detail page URL.

    Returns:
        dict | None: Dictionary with 'images' key (list of URLs) or None on error.
    """
    try:
...
def run_ocr(image_bytes: bytes) -> dict | None:
    ...
    Returns:
        dict | None: Dict with token texts and confidences, or None on error.
    """
...
def auto_label_tokens(tokens: list[str]) -> dict[str, str]:
    ...
    Returns:
        dict[str, str]: Mapping of token indices to label names.
    """
    labels = {}
...
async def process_lots(
    ...
    cursor = conn.execute(
        "SELECT id, title, url FROM lots WHERE url IS NOT NULL LIMIT ?", (
            limit,)
    )
    ...
    print(
        f"2. Train model: python scripts/train_label_classifier.py -i {output_path}")
