...
async def fetch_lot_detail(client: httpx.AsyncClient, url: str) -> dict | None:
    ...
    try:
        ...
        image_pattern = r'https://cdn\.troostwijkauctions\.com/[^"\'>\s]+\.(?:jpg|jpeg|png|webp)'
...
async def process_lots(
    ...
    async with httpx.AsyncClient(
        ...
        for lot_id, title, url in lots:
            ...
            # Process each image
            for img_idx, img_url in enumerate(detail["images"][:3]):  # Max 3 images per lot
                ...
                if not token_data or not token_data["text"]:
                    print(f"    No OCR tokens extracted")
                    continue
