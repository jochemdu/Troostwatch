...
async def create_new_tab(debug_port: int = 9222, url: str = "about:blank") -> dict | None:
...
async def fetch_page_html(ws_url: str, url: str, timeout: float = 30.0) -> str | None:
    ...
    try:
        async with websockets.connect(ws_url) as ws:
            ...
            while not load_complete and (asyncio.get_event_loop().time() - start) < timeout:
                try:
...
async def main():
    ...
    parser.add_argument("--output", type=Path, default=Path("training_data/browser_fetch"))
