...
def generate_model_number() -> str:
    ...
    Returns:
        str: Model number as string.
    """
    patterns = [
        # Samsung style: SM-G991B
        lambda: "SM-"
        + random.choice(["G", "A", "N", "S"])
...
def generate_random_text() -> str:
    ...
    patterns = [
        ...
        lambda: "".join(
            random.choices(
                string.ascii_letters + string.digits + " ", k=random.randint(2, 8)
            )
        ).strip(),
    ]
    return random.choice(patterns)()
...
def generate_image_tokens(
    ...
    Args:
        ean_count (int): Number of EAN codes.
        serial_count (int): Number of serial numbers.
        model_count (int): Number of model numbers.
        part_count (int): Number of part numbers.
        noise_count (int): Number of noise tokens.

    Returns:
        tuple[list[str], list[int], dict[str, str]]: Tokens, confidences, and labels.
    """
    tokens = []
    labels = {}
...
def generate_training_data(num_images: int = 200) -> dict:
    ...
    Args:
        num_images (int): Number of simulated images to generate.

    Returns:
        dict: Training data dict in the expected format.
    """
    images = []
...
def main():
        """
        Main entry point for generating synthetic training data for label classifier.
