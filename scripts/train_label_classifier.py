...
Usage:
    python scripts/train_label_classifier.py --input training_data.json \
...
def prepare_features(token: str, ocr_conf: float, position_ratio: float) -> list[float]:
    ...
    Args:
        ...
        ocr_conf (float): OCR confidence (0-100).
        position_ratio (float): Position in document (0-1).

    Returns:
        list[float]: Feature vector as a list of floats.
    """
    ...
    features.append(1.0 if re.match(
        r"^\d+[A-Z]+\d*$", token) else 0.0)  # Mixed
...
def load_training_data(input_path: Path) -> tuple[np.ndarray, np.ndarray, list[str]]:
    ...
    Args:
        input_path (Path): Path to the training data JSON or JSONL.

    Returns:
        tuple[np.ndarray, np.ndarray, list[str]]: Features, labels, and raw tokens as numpy arrays.
    """
    ...
    if isinstance(data, list):
        ...
        for token_obj in data:
            ...
            all_features.append(prepare_features(
                token_text, conf, position_ratio))
...
def train_model(X: np.ndarray, y: np.ndarray) -> RandomForestClassifier:
    ...
    print("  Mean: {:.3f} (+/- {:.3f})".format(cv_scores.mean(), cv_scores.std() * 2))
    print()
...
def main():
    ...
    print("Loading training data from: {}".format(args.input))
    X, y, tokens = load_training_data(args.input)

    print("Total tokens: {}".format(len(X)))
    ...
    if labeled_count < args.min_samples:
        print("Warning: Only {} labeled samples found.".format(labeled_count))
        print("Need at least {} for reliable training.".format(args.min_samples))
        print()
    ...
    print("\nModel saved to: {}".format(args.output))
