...
def process_tokens_file(filepath: Path) -> tuple[list[dict], dict]:
    ...
    Args:
        filepath (Path): Path to the input tokens file (JSONL).

    Returns:
        tuple[list[dict], dict]: List of labeled token dicts and label distribution stats.
    """
    tokens = []
    stats = {}
...
def main():
        """
        Main entry point for labeling OCR tokens from a file.

        Usage:
            python scripts/label_tokens.py <tokens.jsonl>
        """
    if len(sys.argv) < 2:
