...
class RequestResult:
    ...
    @property
    def ok(self) -> bool:
        return self.text is not None and self.error is None and (self.status is None or 200 <= self.status < 400)
...
class HttpFetcher:
    ...
    def fetch_sync(self, url: str) -> RequestResult:
        ...
        for attempt in range(self.retry_attempts):
            try:
                ...
                return RequestResult(url=url, text=response.text, error=None, status=response.status_code)
            except Exception as exc:  # pragma: no cover - network dependent
                if attempt >= self.retry_attempts - 1:
                    return RequestResult(url=url, text=None, error=str(exc), status=getattr(exc, "response", None) and getattr(exc.response, "status_code", None))
                time.sleep(self._backoff_delay(attempt))
        return RequestResult(url=url, text=None, error="Unknown error", status=None)

    async def _fetch_once_async(self, session: aiohttp.ClientSession, url: str) -> RequestResult:
        host = self._host_from_url(url)
        await self.rate_limiter.wait_async(host)
        for attempt in range(self.retry_attempts):
            try:
                async with session.get(url, headers=self.headers, timeout=self.timeout_seconds) as resp:
                    if resp.status >= 400:
                        raise aiohttp.ClientResponseError(  # pragma: no cover - passthrough
    ...
    async def _fetch_many_asyncio(self, urls: Iterable[str]) -> List[RequestResult]:
        ...
        async def _bounded_fetch(session: aiohttp.ClientSession, url: str) -> RequestResult:
