"""Debugging subcommands for Troostwatch.
...
``python -m troostwatch.cli debug`` to see the available subcommands.
...
from ..debug_tools import db_stats, db_integrity, db_view
...
def view_cmd(ctx: click.Context, table: str, limit: int) -> None:
    ...
    Example::
        ...
        python -m troostwatch.cli debug view buyers
    """
    db_path = ctx.obj["db_path"]
    with get_connection(db_path) as conn:
        try:
